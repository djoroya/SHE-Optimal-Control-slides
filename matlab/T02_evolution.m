...
plot(tspan,ft,'LineWidth',2,'Marker','.','MarkerSize',20);
...
cla
hold on
plot(xt(end,1)-xt(:,1),xt(end,2) - xt(:,2),'LineWidth',2,'Marker','o')
...
v = VideoWriter(fullfile(path,'peaks-rev-fewnt'),'MPEG-4');
v.FrameRate = 0.8;
open(v);
%
%
for it = 1:Nt
   ...
   jplot.YData = ft(it);
   %
   p1plot.XData = [ xt(end,1) - xt(it,1),xt(end,1) - xt(it,1) -  dt*(2/pi)*cos(tspan(it))];
   p1plot.YData = [ xt(end,2) - xt(it,2),xt(end,2) - xt(it,2) -  dt*(2/pi)*sin(tspan(it))];
  
   p2plot.XData = [ xt(end,1) - xt(it,1),xt(end,1) - xt(it,1) +  dt*(2/pi)*cos(tspan(it))];
   p2plot.YData = [ xt(end,2) - xt(it,2),xt(end,2) - xt(it,2) +  dt*(2/pi)*sin(tspan(it))];
